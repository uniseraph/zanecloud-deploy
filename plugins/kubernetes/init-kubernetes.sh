...
wget https://github.com/kubernetes/kubernetes/releases/download/v1.6.12/kubernetes.tar.gz
...
sed -i -e "s#master0#${MASTER_IP}#g" /etc/kubernetes/config
sed -i -e "s#master0#${MASTER_IP}#g" /etc/kubernetes/apiserver
sed -i -e "s#master0#${MASTER_IP}#g" /etc/kubernetes/kubelet
